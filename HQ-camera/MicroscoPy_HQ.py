# Modified code for the HQ camera

# MICROSCOPE APP
# Program to control camera settings, microscope stepper motors, and LED illumination from keyboard
# Camera: Pi camera v2
# Enable the camera: Preferences --> Raspberry Pi Configuration --> Interfaces --> Camera --> Enable --> Restart
# Increase GPU memory (Raspberry Pi 4, max. camera resolution): Preferences --> Raspberry Pi Configuration --> Performance --> GPU memory --> Change from 128 to 144
...
HighResolution = True # True: (4056, 3040), False: (1920, 1080)
    ...
    if key == Key.tab:
        if recording_mode == False:
            ...
            if HighResolution == True:
                camera.resolution = (4056, 3040) # Pi camera v2 max resolution, 8MP
            else:
